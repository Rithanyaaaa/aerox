  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-white shadow-md top-0' : 'bg-white/95 backdrop-blur-sm md:top-8'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <button onClick={() => handleNavClick('/')} className="flex items-center group">
            <img src="/logo.png" alt="AEROX Logo" className="h-14 w-auto object-contain" />
          </button>
            ...
            {navLinks.map((link: NavLink) => (
              ...
              >
                <button
                  ...
                  className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-colors relative group ${
                ...
                <AnimatePresence>
                  {link.dropdown && activeDropdown === link.label && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 w-52 bg-white border border-gray-200 shadow-xl py-2"
                    >
                      {link.dropdown.map((item) => (
                        <button
                          key={item.label}
                          onClick={() => handleNavClick(item.href)}
                          className="block w-full text-left px-4 py-2.5 text-sm text-gray-600 hover:text-primary hover:bg-primary/5 transition-colors"
                        >
                          {item.label}
                        </button>
                      ))}
